...
public class CentralHarnessCSVMapping : MonoBehaviour
{
    public bool Node2;
    public bool Node3;
    public bool Node4;
    ...
    void Start()
    {
        ...
        AllEndConnAndPins = new EndConnectorAndPin[lines.Length + 10]; //keeping track of all end point objects for reverse mapping
        ...
        if (Csv_name == "Airbag") { ExtendedWireColumn = 16; }
        if (Csv_name == "Background lighting (QQ1)") { ExtendedWireColumn = 20; }
        if (Csv_name == "Climatic (9AB)") { ExtendedWireColumn = 21; }
    ...
    public void StartMapping()
    {
        ...
        for (int i = 1, j = 1; i < lines.Length; i++, j++) // go through line by line in the csv
        {
            ...
            if (lines[i].Trim() == ",,,,,,,,,,,,,,,,,,,,") // if found an empty line with exact number of commas in the csv, consider that block and start mappimng
            {
                ...
                if (toBeIncluded) // if that connector has to be included, then only proceed
                {
                    ...
                    if (splitData[4] != "" && GameObject.Find(splitData[4]) != null && splitData[4] != "SA FUSE BOX" && splitData[4] != "SB FUSE BOX" && splitData[4] != "SC FUSE BOX") // add that connector in the lists
                    {
                        ConnDesig.Add(GameObject.Find(splitData[4]));
                        ConnName.Add(splitData[1]);
                        if (splitData[2] != "")
                        {
                            CompDesig.Add(splitData[2]);
                        }
                    }
                    else if (splitData[4] == "" || splitData[4] == "Lug" || splitData[4] != "SA FUSE BOX" || splitData[4] != "SB FUSE BOX" || splitData[4] != "SC FUSE BOX")
                    {
                ...
                for (int l = 0; l < j - 1; l++)
                {
                    ...
                    {
                        if ((Csv_name == "DQ200(0EG)" && (splitData[2] == "SB15" || splitData[2] == "SC53")) || (Csv_name == "Mobile Telephone Systems" && splitData[2] == "N-369") || (Csv_name == "Climatronic" && (splitData[2] == "N-B317" || splitData[2] == "SB")) || (Csv_name == "Automatic GB 09G (PR - 0ER)" && (splitData[2] == "SB15" || splitData[2] == "SC53")) || splitData1[10] == "" && splitData1[1] != "" && splitData1[1] != "Sliding Sunroof Motor" && splitData1[1] != "Radiator Fan Control Unit" && splitData[2] != "J794" && splitData[2] != "J126" && splitData[2] != "EX21" && splitData[2] != "J255" && (splitData[2] != "J519" && Csv_name == "Centre Console Switches (4D0)") && (splitData[2] != "J104" && Csv_name == "Centre Console Switches (4D0)") || splitData[2] == "H")
                        {
                            //Debug.Log(splitData[2]);
                            conn.wires[tempInt1 - 1].nodes = new Node[2];
                            conn.wires[tempInt1 - 1].nodes[0] = new Node();
                        ...
                        else if (Csv_name == "Automatic GB 09G (PR - 0ER)" && splitData[5] == "10" && splitData[4] == "T10r" || (Csv_name == "Basic Equipment" && (splitData[2] == "SC58" || splitData[2] == "J329")))
                        {
                        ...
                        else if (Csv_name == "Basic Equipment" && ((splitData[5] == "8" && splitData[4] == "T10xc") || (splitData[5] == "1" && splitData[4] == "T2e")))
                        {
                            conn.wires[tempInt1 - 1].nodes = new Node[2];
                            conn.wires[tempInt1 - 1].nodes[0] = new Node();
                            conn.wires[tempInt1 - 1].nodes[1] = new Node();
                ...
                for (int l = 1; l < j; l++)
                {
                    ...
                    if (Csv_name == "Basic Equipment" && conn.name == "T2bv" && tempInt == 1)
                    {
                        conn.wires[tempInt - 1].wireNumber = 0;
                    }
                    if (Csv_name == "Airbag" && ExtendedWireColumn != 0 && splitData[ExtendedWireColumn] != "")  // only for airbag cfd which has extra values
                    {
                    ...
                    if (Csv_name == "Background lighting (QQ1)" && ExtendedWireColumn != 0 && splitData[ExtendedWireColumn] != "")  // only for Background lighting cfd which has extra values
                    {
                        conn.wires[tempInt - 1].nodes[0].ExtraComponent = new ExtendedWireComponent();
                        //Debug.Log(splitData[23]);
                        if (splitData[23] != "")
                        {
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.crossSection = System.Convert.ToDouble(splitData[23]);
                        }
                        if (splitData[22] != "")
                        {
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.colorCode = splitData[22];
                        }
                        if (splitData[24] != "")
                        {
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.node = splitData[24];
                        }
                        conn.wires[tempInt - 1].nodes[0].ExtraComponent.endConnectorDesignation = splitData[20];
                        conn.wires[tempInt - 1].nodes[0].ExtraComponent.pinNumber = System.Convert.ToInt32(splitData[21]);

                        if (splitData[27] != "")
                        {
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.extraConnectorDesignation = splitData[27];
                        }
                        ...
                        if (splitData[28] != "")
                        {
                            //Debug.Log(splitData[28]);
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.extraPinNumber = System.Convert.ToInt32(splitData[28]);
                        }

                    }

                    if (Csv_name == "Climatic (9AB)" && ExtendedWireColumn != 0 && splitData[ExtendedWireColumn] != "")  // only for Background lighting cfd which has extra values
                    {
                        conn.wires[tempInt - 1].nodes[0].ExtraComponent = new ExtendedWireComponent();
                        //Debug.Log(splitData[23]);
                        if (splitData[25] != "")
                        {
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.crossSection = System.Convert.ToDouble(splitData[25]);
                        }
                        if (splitData[24] != "")
                        {
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.colorCode = splitData[24];
                        }
                        if (splitData[26] != "")
                        {
                            conn.wires[tempInt - 1].nodes[0].ExtraComponent.node = splitData[26];
                        }
                        conn.wires[tempInt - 1].nodes[0].ExtraComponent.endConnectorDesignation = splitData[21];
                        conn.wires[tempInt - 1].nodes[0].ExtraComponent.pinNumber = System.Convert.ToInt32(splitData[23]);
                ...
                for (int w = 0; w < conn.wires.Length; w++) // assigning node values inside the Wires
                {
                    ...
                    if (tempInt != 0 && conn.wires[tempInt - 1] != null && conn.wires[tempInt - 1].nodes != null)
                    {
                        for (int n = 0; n < conn.wires[tempInt - 1].nodes.Length; n++)
                        {
                            ...
                            if (Csv_name != "Ent&Start Auth (4I3)" && Csv_name != "Power Steering" && Csv_name != "LED Headlights(8IY)" && Csv_name != "Rear Parking Aid(PDC, 7X3)" && Csv_name != "Multifunction Steering Wheel" && Csv_name != "Centre Console Switches (4D0)" && Csv_name != "Centre Console Switches (4D3)" && Csv_name != "Elec Adj Ext Mirror" && Csv_name != "Radio System(8UK,8RU)" &&
                                Csv_name != "Radio System (8UW)" && Csv_name != "Radio System(8UK,8RL)" && Csv_name != "Power Steering" &&
                                Csv_name != "Automatic GB 09G (PR - 0ER)" && Csv_name != "Wireless Data Transfer" && Csv_name != "DQ200(0EG)" && Csv_name != "Climatic (9AB)" && Csv_name != "Background lighting (QQ1)" && splitData[25] != "" && Csv_name != "Tail LGT LED(8SA)" && Csv_name != "Tail LGT LED(8VG)" && Csv_name != "Halogen Headlights(8ID)")
                            {
                                conn.wires[tempInt - 1].nodes[n].details = splitData[25];
                            }
                            else if (Csv_name == "LED Headlights(8IY)" || Csv_name == "Halogen Headlights(8ID)" || Csv_name == "Ent&Start Auth (4I3)" || Csv_name == "Power Steering" || Csv_name == "LED Headlights(8IY)" || Csv_name == "Rear Parking Aid(PDC, 7X3)" || Csv_name == "Multifunction Steering Wheel" || Csv_name == "Centre Console Switches (4D3)" || Csv_name == "Radio System(8UK,8RU)" || Csv_name == "Radio System(8UK,8RL)" || Csv_name == "Radio System (8UW)" || Csv_name == "Tail LGT LED(8VG)" || Csv_name == "Tail LGT LED(8SA)" || Csv_name == "Centre Console Switches (4D0)" || Csv_name == "Elec Adj Ext Mirror" || Csv_name == "Auto anti-dazzle Mirror" && splitData[28] != "" && Csv_name != "Background lighting (QQ1)" && Csv_name != "Climatic (9AB)" && Csv_name != "DQ200(0EG)" && Csv_name != "Wireless Data Transfer" && Csv_name != "Automatic GB 09G (PR - 0ER)")
                            {
                                conn.wires[tempInt - 1].nodes[n].details = splitData[28];
                            }
                            else if (Csv_name != "Automatic GB 09G (PR - 0ER)" && Csv_name == "Background lighting (QQ1)" && Csv_name != "Climatic (9AB)" && Csv_name != "DQ200(0EG)" && Csv_name != "Wireless Data Transfer")
                            {
                            ...
                            if (Csv_name == "Basic Equipment" || Csv_name == "Reversing Camera (KA1)" || Csv_name == "Seat Belt Warning System" || Csv_name == "Seat Ventilation (4D3)" || Csv_name == "CS Power Window (4R1)" || Csv_name == "CS Power Window (4R4)" || Csv_name == "Wireless Data Transfer" && splitData[31] != "")
                            {
                            ...
                            {
                                if (Csv_name == "Wireless Data Transfer" || Csv_name == "CS Power Window (4R1)" || Csv_name == "CS Power Window (4R4)" || Csv_name == "Basic Equipment" || Csv_name == "Reversing Camera (KA1)" || Csv_name == "Seat Belt Warning System" || Csv_name == "Seat Ventilation (4D3)")
                                {
                                    conn.wires[tempInt - 1].nodes[n].endComponentDesignation = splitData[20];
                                }
                                else if (Csv_name == "Rear Parking Aid(PDC, 7X3)" || Csv_name == "Background lighting (QQ1)" || Csv_name == "Elec Adj Ext Mirror" || Csv_name == "Halogen Headlights(8ID)" || Csv_name == "LED Headlights(8IY)" || Csv_name == "Tail LGT LED(8SA)" || Csv_name == "Tail LGT LED(8VG)" || Csv_name == "Centre Console Switches (4D0)" || Csv_name == "Power Steering" || Csv_name == "Radio System (8UW)" || Csv_name == "Radio System(8UK,8RL)" || Csv_name == "Radio System(8UK,8RU)")
                                {
                                ...
                                {
                                    conn.wires[tempInt - 1].nodes[n].endComponentDesignation = splitData[19];
                                }
                                else
                                {
                                ...
                                //Debug.Log(splitData[19]);
                                conn.wires[tempInt - 1].nodes[n].endPointDesignation = splitData[11];
                                if (Csv_name != "Basic Equipment" && Csv_name != "Reversing Camera (KA1)" && Csv_name != "Seat Belt Warning System" && Csv_name != "Seat Ventilation (4D3)" && Csv_name != "CS Power Window (4R1)" && Csv_name != "CS Power Window (4R4)" && Csv_name != "Wireless Data Transfer" && splitData[1] != "Fuse 7.5" && Csv_name != "Centre Console Switches (4D0)" && Csv_name != "Centre Console Switches (4D3)" && Csv_name != "Multifunction Steering Wheel" && Csv_name != "Rear Parking Aid(PDC, 7X3)" && Csv_name != "Background lighting (QQ1)" && Csv_name != "Halogen Headlights(8ID)" && Csv_name != "LED Headlights(8IY)" && Csv_name != "Ent&Start Auth (4I3)" && Csv_name != "Elec Adj Ext Mirror" && Csv_name != "Tail LGT LED(8SA)" && Csv_name != "Tail LGT LED(8VG)" && Csv_name != "Power Steering" && Csv_name != "Climatic (9AB)" && Csv_name != "Radio System (8UW)" && Csv_name != "Radio System(8UK,8RL)" && Csv_name != "Radio System(8UK,8RU)" && Csv_name != "CS Power Window (4R1)" && Csv_name != "CS Power Window (4R4)" && Csv_name != "Basic Equipment" && Csv_name != "Seat Ventilation (4D3)" && Csv_name != "Seat Belt Warning System" && Csv_name != "Reversing Camera (KA1)" && Csv_name != "DQ200(0EG)" && Csv_name != "Automatic GB 09G (PR - 0ER)")
                                {
                                    if (splitData[11] != "")
                                    {
                                ...
                                else if (Csv_name != "Basic Equipment" && Csv_name != "Reversing Camera (KA1)" && Csv_name != "Seat Belt Warning System" && Csv_name != "Seat Ventilation (4D3)" && Csv_name != "CS Power Window (4R1)" && Csv_name != "CS Power Window (4R4)" && Csv_name != "Automatic GB 09G (PR - 0ER)" && Csv_name != "DQ200(0EG)" && Csv_name != "Climatic (9AB)" && Csv_name != "Wireless Data Transfer" && (splitData[1] == "Fuse 7.5" || Csv_name == "Centre Console Switches (4D0)" || Csv_name == "Centre Console Switches (4D3)" || Csv_name == "Multifunction Steering Wheel" || Csv_name == "Rear Parking Aid(PDC, 7X3)" || Csv_name == "Background lighting (QQ1)" || Csv_name == "Halogen Headlights(8ID)" || Csv_name == "LED Headlights(8IY)" || Csv_name == "Ent&Start Auth (4I3)" || Csv_name == "Elec Adj Ext Mirror" || Csv_name == "Tail LGT LED(8SA)" || Csv_name == "Tail LGT LED(8VG)" || Csv_name == "Power Steering" || Csv_name == "Radio System (8UW)" || Csv_name == "Radio System(8UK,8RL)" || Csv_name == "Radio System(8UK,8RU)" || Csv_name == "CS Power Window (4R1)" || Csv_name == "CS Power Window (4R4)" || Csv_name == "Basic Equipment" || Csv_name == "Seat Ventilation (4D3)" || Csv_name == "Seat Belt Warning System" || Csv_name == "Reversing Camera (KA1)"))
                                {
                                    if (splitData[15] != "")
                                    {
                                ...
                                else if (Csv_name == "Basic Equipment" || Csv_name == "Reversing Camera (KA1)" || Csv_name == "Seat Belt Warning System" || Csv_name == "Seat Ventilation (4D3)" || Csv_name == "Wireless Data Transfer" || Csv_name == "CS Power Window (4R1)" || Csv_name == "CS Power Window (4R4)")
                                {
                                    if (splitData[18] != "")
                                    {
                                    ...
                                    {
                                        if (splitData[20] != "" && GameObject.Find(splitData[20]) && (Csv_name != "Basic Equipment" || splitData[20] == "SB10" || splitData[20] == "811" || splitData[20] == "44" || splitData[20] == "SC30" || splitData[20] == "SC36"))
                                        {
                                ...
                                else if (Csv_name == "DQ200(0EG)" || Csv_name == "Climatic (9AB)" || Csv_name == "Automatic GB 09G (PR - 0ER)")
                                {
                                    ...
                                    if (splitData[16] != "")
                                    {
                                        ...
                                        //if(conn.name == "T73a") { Debug.Log(conn.wires[tempInt - 1].nodes[n].endPointObj); }
                                    }
                                ...
                                if (splitData[10] != "" && double.TryParse(splitData[10], out double abc))
                                {
                                    //Debug.Log(splitData[10] + " " + splitData[4]);
                                    conn.wires[tempInt - 1].nodes[n].nodeCrossSection = System.Convert.ToDouble(splitData[10]);
                                }
                                else
                                {
                                ...
                                if (Node2) // if node2 is true, it means extra node is present, filling values here
                                {
                                    if (splitData[11] != "")
                                    {
                                        ...
                                        if (splitData[12] != "")
                                        {
                                            conn.wires[tempInt - 1].nodes[n].node2CrossSection = System.Convert.ToDouble(splitData[12]);
                                        }

                                        if (splitData[13] != "")
                                        {
                                            conn.wires[tempInt - 1].nodes[n].node2ColorCode = splitData[13];
                                        }
                                    }
                                }
                                ...
                                if (Csv_name != "DQ200(0EG)" && Csv_name != "Climatic (9AB)" && splitData[11] != null && GameObject.Find(splitData[11]) && conn.wires[tempInt - 1].nodes[n].endPointObj == null)
                                {
                                    conn.wires[tempInt - 1].nodes[n].endPointObj = GameObject.Find(splitData[11]);
                                }
                                else if (Csv_name != "DQ200(0EG)" && Csv_name != "Climatic (9AB)" && conn.wires[tempInt - 1].nodes[n].endPointObj == null)
                                {
                                    conn.wires[tempInt - 1].nodes[n].endPointObj = GameObject.Find(splitData[14]);
                                }
                                ...
                                {
                                    GameObject[] tempNode = FindObjectOfType<AllNodes>().AllNodess;
                                ...
                                //Debug.Log(conn.wires[tempInt - 1].nodes[n].endPointObj);
                                if (conn.wires[tempInt - 1].nodes[n].endPointObj != null && conn.wires[tempInt - 1].nodes[n].endPointObj.tag != "Node" && conn.wires[tempInt - 1].nodes[n].endPointObj.name != "811" && conn.wires[tempInt - 1].nodes[n].endPointObj.name != "44" && conn.wires[tempInt - 1].nodes[n].endPointObj.name != "43")
                                {
                                    if (splitData[18] != ""
                                        && (Csv_name == "Wireless Data Transfer"
                                        || Csv_name == "CS Power Window (4R1)"
                                        || Csv_name == "CS Power Window (4R4)"
                                        || Csv_name == "Seat Ventilation (4D3)"
                                        || Csv_name == "Seat Belt Warning System"
                                        || Csv_name == "Reversing Camera (KA1)"
                                        || Csv_name == "Basic Equipment"))
                                    {
                                        Debug.Log(Csv_name+" "+conn.wires[tempInt - 1].nodes[n].endPointObj + " " + splitData[18]);
                                        ...
                                        || Csv_name == "Radiator Fan"
                                        || Csv_name == "Mobile Telephone Systems"
                                        || Csv_name == "Climatronic"))
                                    {
                                        ...
                                        || (Csv_name == "Background lighting (QQ1)" && splitData[17] != "43")
                                        || (Csv_name == "Elec Adj Ext Mirror" && splitData[17] != "43")
                                        ...
                                        || (Csv_name == "Tail LGT LED(8VG)" && splitData[17] != "SC16"
                                            && splitData[17] != "SC35" && splitData[17] != "SB9"
                                            && splitData[17] != "SC40" && splitData[17] != "SB")
                                        || (Csv_name == "Power Steering" && splitData[17] != "811")
                                        || (Csv_name == "Ent&Start Auth (4I3)" && splitData[17] != "811"
                                            && splitData[17] != "43" && splitData[17] != "44")
                                        || Csv_name == "Radio System (8UW)"
                                        || Csv_name == "Radio System(8UK,8RL)"
                                        || Csv_name == "Radio System(8UK,8RU)")

                                    {
                                        if (conn.wires[tempInt - 1].nodes[n].endPointObj.tag != "Node" && !conn.wires[tempInt - 1].nodes[n].endPointObj.name.StartsWith("SA") && !conn.wires[tempInt - 1].nodes[n].endPointObj.name.StartsWith("SB") && !conn.wires[tempInt - 1].nodes[n].endPointObj.name.StartsWith("SC") && !conn.wires[tempInt - 1].nodes[n].endPointObj.name.StartsWith("5"))
                                        {
                                            //Debug.Log(Csv_name+" "+conn.wires[tempInt - 1].nodes[n].endPointObj + " " + splitData[15]);
                                    ...
                                    //Debug.Log("Final endptObj: " + conn + " " + " " + tempInt + conn.wires[tempInt - 1].nodes[n].endPointPinNum + " " + conn.wires[tempInt - 1].nodes[n].endPointObj);
                                }
                                LC++;
                            }
        ...
        for (int v = 0; v < AllEndConnAndPins.Length; v++) // reverse mapping
        {
            for (int w = 0; w < ConnDesig.Count; w++)
            {
                if (AllEndConnAndPins[v].endConnector != null && AllEndConnAndPins[v].endConnector.name == ConnDesig[w].name)
                {
                    ...
                    if (!ConnDesig[w].name.StartsWith("SB") && !ConnDesig[w].name.StartsWith("SC") && ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].wireNumber == 0)
                    {
                        ...
                        //Debug.Log("Checking for " + ConnDesig[w].name + " " + AllEndConnAndPins[v].endPinNum);

                        for (int x = 0; x < AllEndConnAndPins[v].ReverseWire.nodes.Length; x++)
                        {
                            if (AllEndConnAndPins[v].ReverseWire.nodes[x].endPointPinNum == AllEndConnAndPins[v].endPinNum)
                            {
                                //Debug.Log("Node copying working for " + ConnDesig[w].name + " " + AllEndConnAndPins[v].endPinNum);
                                //ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0] = AllEndConnAndPins[v].ReverseWire.nodes[x];

                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].toInclude = AllEndConnAndPins[v].include;

                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].NodeSTR = AllEndConnAndPins[v].ReverseWire.nodes[x].NodeSTR;
                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].nodeCrossSection = AllEndConnAndPins[v].ReverseWire.nodes[x].nodeCrossSection;
                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].nodeColorCode = AllEndConnAndPins[v].ReverseWire.nodes[x].nodeColorCode;

                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].Node2STR = AllEndConnAndPins[v].ReverseWire.nodes[x].Node2STR;
                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].node2CrossSection = AllEndConnAndPins[v].ReverseWire.nodes[x].node2CrossSection;
                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].node2ColorCode = AllEndConnAndPins[v].ReverseWire.nodes[x].node2ColorCode;

                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].Node3STR = AllEndConnAndPins[v].ReverseWire.nodes[x].Node3STR;
                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].node3CrossSection = AllEndConnAndPins[v].ReverseWire.nodes[x].node3CrossSection;
                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].node3ColorCode = AllEndConnAndPins[v].ReverseWire.nodes[x].node3ColorCode;

                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].endComponentDesignation = AllEndConnAndPins[v].startConnector.GetComponent<Connector>().componentDesignation;
                                ...
                                ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].details = AllEndConnAndPins[v].ReverseWire.nodes[x].details;
                                ...
                                //Debug.Log(ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].nodes[0].endPointObj + " " + ConnDesig[w].GetComponent<Connector>().wires[AllEndConnAndPins[v].endPinNum - 1].wireNumber);
                                if (!AllEndConnAndPins[v].startConnector.name.StartsWith("SC") && !AllEndConnAndPins[v].startConnector.name.StartsWith("SB") && !AllEndConnAndPins[v].startConnector.name.StartsWith("SA") && !AllEndConnAndPins[v].startConnector.name.StartsWith("J329"))
                                {
        ...
        ResetConnMat();
        FindObjectOfType<ConnectedWire>().DisableNodes();
    }
            ...
            for (int j = 0; j < ConnDesig.Count; j++)
            {
                if (AllConnectors.transform.GetChild(i).name == ConnDesig[j].name)
                {
                    ...
                    foreach (Renderer r in rend)
                    {
                        ...
                        {
                            Debug.Log(r.materials[ij] + " " + ij);
                            r.materials[ij] = CentralHarnessConns;
                        }*/
                        ...
                        foreach(Material m in r.materials)
                        {
                            ...
                            if(r.materials.Length > 3)
                            {
                                ...
                                mats[3] = new Material(CentralHarnessConns);
                                mats[2] = new Material(CentralHarnessConns);
                                mats[1] = new Material(CentralHarnessConns);
