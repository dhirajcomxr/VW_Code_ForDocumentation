...
namespace UnityEngine.UI.Extensions {
    ...
    public class AutoCompleteComboBox : MonoBehaviour {
        ...
        public List<string> _panelItems; //items that will get shown in the drop-down
        public List<string> _prunedPanelItems; //items that used to show in the drop-down
        ...
        private Scene scene;
        public GameObject CurrentSelectedCFD;
        private string cfdString;
        ...
        public void Start() {
            //         Debug.Log("Started");
            //SelectableConns = SB.SelectableConns;
            scene = SceneManager.GetActiveScene();

            if (scene.name == "VariantSelectionForCentralHarness") {
                //cfdString = CurrentSelectedCFD.GetComponent<Text>().text;
                if (cfdString != "") {
                    PlayerPrefs.SetString("cfd", cfdString);
            ...
            LD1 = FindObjectOfType<LineDetector>();

            //SelectableConns = _1L.ConnDesig;
        ...
        private void Update() {
            if (AvailableOptions.Count == 0) {
                //Debug.Log(SB.SelectableConns.Count);
                //cfdString = 

                //AvailableOptions.AddRange(_1L.ConnName);
                foreach (GameObject g in SB.SelectableConns) {
                    //Debug.Log(g.name);
                    if (g.name != "") {
                        AvailableOptions.Add(g.name);


                    }
                }
                foreach (string g in SB.SelectableConnsName) {
                    //Debug.Log(g.name);
                    if (g != "") {
                        AvailableOptions.Add(g);
                ...
                RedrawPanel();
                //AvailableOptions.AddRange(_1L.ConnDesig);
            }
        ...
        private void OnItemClicked(string item) {
            ...
            foreach (GameObject g in SelectableConns) {
                ...
                if (Connectorname.ToLower() == item.ToLower()
                    || ConnectorDesign.ToLower() == item.ToLower()
                    || ComponentDesign.ToLower() == item.ToLower()
                    || item.ToLower().Contains("(" + g.name.ToLower() + ")")
                    || item.ToLower() == g.name.ToLower()) {

                    if (LD != null) {
                        LD.SelectConnector(g);
                    }
                    else if(LD1!=null){
                        LD1.SelectConnector(g);
                    }
                    else {
                        Debug.Log("Lagg gaye");
                    }
            ...
            if (scene.name == "VariantSelectionForCentralHarness") {
                if (cfdString != "") {
                    for (int m = 0; m < AvailableOptions.Count; m++) {
                        if (_mainInput.text == AvailableOptions[m].ToLower()) {
                            cfdString = AvailableOptions[m];
                        }
                    }
                    PlayerPrefs.SetString("cfd", cfdString);
                }
        ...
        public void VariantSelected() {

            PlayerPrefs.SetInt("Variant", variantDropDown.value);
        }

        public void LoadMainMenuScene() {
            SceneManager.LoadScene("VariantSelectionForCentralHarness");
        }
    }
}
